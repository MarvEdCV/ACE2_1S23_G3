...
var max;
function setup() {
    axios.get('http://localhost:4010/api/presion-barometrica', {responseType: 'json'}).then(function(res) {
        ...
        if(res.status==200) {
            ...
            for (var i = 0; i < numberOfRows; i++) {
                arr.push(data[i].presion_barometrica);
            }
            max = Math.max(...arr);
  ...
  function draw() {
    ...
    for (var i = 0; i < numberOfRows; i++) {
      ...
      //pull numbers
      budgetValues[i] = data[i].presion_barometrica;
      //draw graph
